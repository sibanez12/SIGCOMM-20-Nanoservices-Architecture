...
\begin{figure}
  ...
  \label{fig:nanoPU}
\end{figure}
\shahbaz{we discuss CSR a lot in the later part of this section. maybe we should point out where these CSR register sit in Figure 3?}
...
\begin{itemize}
    ...
    % The NIC is responsible for exposing a message interface (as opposed to a packet interface) to the core(s) it is connected to.
...
\nick{This is a very good point. WHat if we rename the NIC Datapath to the NIC Packet Datapath and the NIC-CPU INterface to the NIC Message Datapath, to indicate that it handles messages for the CPU.}
